...
- Numerical: age, jaccard similarity measure, duration trace, length of stay in hospital, period, number of admission prior emergency, number of admission prior inhospital.
:::
::: {.callout-tip}
...
count(*) filter (where type_ischemic_stroke_bl = TRUE) as n_episodes_with_ischemic_stroke,
count(*) filter (where inhospital_fibrinolysis_bl = TRUE OR thrombolysis_emergency_dt is NOT NULL) as n_episodes_with_fibrinolysis,
count(*) filter (where inhospital_thrombectomy_bl = TRUE) as n_episodes_with_thrombectomy,
...
descriptive_values <- descriptive_values %>% mutate(
  perc_ischemic = round((n_episodes_with_ischemic_stroke/n_episodes)*100,2),
  united_ischemic =paste0(n_episodes_with_ischemic_stroke, ' (',perc_ischemic,'%)' ),
  perc_fibrinolysis = round((n_episodes_with_fibrinolysis/n_episodes)*100,2),
  united_fibrinolysis =paste0(n_episodes_with_fibrinolysis, ' (',perc_fibrinolysis,'%)' ),
  perc_thrombectomy = round((n_episodes_with_thrombectomy/n_episodes)*100,2),
  united_thrombectomy =paste0(n_episodes_with_thrombectomy, ' (',perc_thrombectomy,'%)' ),
  perc_exitus = round((n_patient_exitus/n_patient_distinct)*100,2),
  united_exitus =paste0(n_patient_exitus, ' (',perc_exitus,'%)' ),
...
descriptive_values_ <- descriptive_values %>% dplyr::select(ccaa_cd,median_age,iqr_age,n_episodes,n_patient_distinct,
                                                            united_men,united_women,united_fibrinolysis,united_thrombectomy,
                                                            united_exitus,united_ischemic)
descriptive_values_ <- melt(descriptive_values_,id=NULL) 
...
descriptive_values_$variable[descriptive_values_$variable %in% 'n_episodes'] <- 'N episodes'
...
descriptive_values_$variable[descriptive_values_$variable %in% 'united_ischemic'] <- 'N episodes with ischemic stroke'
descriptive_values_$variable[descriptive_values_$variable %in% 'united_fibrinolysis'] <- 'N episodes with fibrinolysis'
descriptive_values_$variable[descriptive_values_$variable %in% 'united_thrombectomy'] <- 'N episodes with thrombectomy'
descriptive_values_$variable[descriptive_values_$variable %in% 'united_exitus'] <- 'N patient with exitus (unique) date (death)'
...
descriptive_traces_ <- descriptive_traces %>% group_by(`case:concept:name`) %>% 
  summarise(dur_trace = as.numeric(max(`time:timestamp`,na.rm=TRUE) - min(`time:timestamp`,na.rm=TRUE)),
...
::: {.callout-tip}
These traces correspond to those shown in @fig-survival_descriptive. Thus, **Trace: 1** corresponds to **trace 1** in the referenced figure. The number (identifier) of the trace corresponds to the position it occupies in a ranking of the most frequent traces, so that **Trace: 1** is the most frequent.
:::


```{r, echo = FALSE, warning = FALSE,fig.cap="Jaccard's similarity for the 10 most frequent traces"}
...
p$rank_trace <- as.numeric(p$rank_trace)

plot <- p[order(p$rank_trace),] %>% 
    trace_explorer(n_traces = 10, plotly = FALSE,coverage_labels = c('cumulative'),show_labels = FALSE)
...
plot + labs(title='Jaccard similarity measured (10 most frequent traces)') + 
  geom_text(aes(x=1,label=paste0('Trace: ',rank_trace.y)), vjust=0) +
...
predict_next_activity_function <- function(database_path,n_epochs_fit){
  ...
  # Intervention bl remove to not include in model
  set.seed(1234)
  perc_train_set <- 0.7
    
  data_set_interv <- p %>% filter(intervention_bl %in% 1)
    
  data_set_nointerv <- p %>% filter(intervention_bl %in% 0)
    
  if(length(unique(data_set_interv$patient_id))>=(length(unique(data_set_nointerv$patient_id)))){
      set.seed(1234)
      size_df <- length(unique(data_set_nointerv$patient_id))
      size_sample <- floor(0.7 * size_df)
      patient_id_nointerv <- data_set_nointerv[!duplicated(data_set_nointerv$patient_id),]
      patient_id_nointerv <- sample(patient_id_nointerv$patient_id, size_sample,replace = FALSE)
      train_df_nointerv <- data_set_nointerv %>% filter(patient_id %in% patient_id_nointerv)
      ...
      set.seed(1234)
      size_df <- length(unique(data_set_interv$patient_id))
      size_sample <- floor(0.7 * size_df)
      patient_id_interv <- data_set_interv[!duplicated(data_set_interv$patient_id),]
      patient_id_interv <- sample(patient_id_interv$patient_id, size_sample,replace = FALSE)
      train_df_interv <- data_set_interv %>% filter(patient_id %in% patient_id_interv)
      patient_id_ <- data_set_nointerv[!duplicated(data_set_nointerv$patient_id),] 
      patient_id_ <- sample(patient_id_$patient_id, length(unique(train_df_interv$patient_id)),replace=FALSE)
  ...
  rm(test_df_interv, test_df_nointerv,train_df_nointerv,train_df_interv)
  
  train_df <- prepare_examples(train_df, task = "next_activity",
                                      features = c('hospital_cd', 'healthcare_area_cd',
                                                   'age_nm', 'sex_cd','ct_inhospital_bl',
                                    'mri_inhospital_bl','type_admission_cd',
                                    'antiarrhythmics_prescription_bl','antihypertensive_prescription_bl',
                                    'antiaggregants_prescription_bl','fibrinolitics_prescriptions_bl',
                                    'heart_failure_bl','hypertension_bl','diabetes_bl',
                                    'atrial_fibrillation_bl','valvular_disease_bl'))
  test_df <- prepare_examples(test_df, task = "next_activity",
                                      features = c('hospital_cd', 'healthcare_area_cd',
                                                   'age_nm', 'sex_cd','ct_inhospital_bl',
                                    'mri_inhospital_bl','type_admission_cd',
                                    'antiarrhythmics_prescription_bl','antihypertensive_prescription_bl',
  ...
  hist <- fit(object = model, train_data = train_df, epochs = n_epochs_fit,callbacks = NULL)
  ...
  eval <- model %>% evaluate(test_df)
  eval <- round(eval,2)
  predictions <- model %>% predict(test_data = test_df, 
...
SELECT
	...
	'sum') as sum_comorbidities,
	CASE
		WHEN hospital_admission_date_dt IS NOT NULL THEN DATEDIFF('day',
		hospital_admission_date_dt,
		hospital_discharge_date_dt)
		ELSE 0
	END AS los_hospital,
	CASE
		WHEN admission_to_observation_ward_dt IS NOT NULL THEN DATEDIFF('day',
		admission_to_observation_ward_dt,
		discharge_from_emergency_dt)
		ELSE 0
	END AS los_observation_discharge_emergency
from
	main.patient_view),
traces_info as (
select
	...
	b.sum_comorbidities,
	b.los_hospital,
	b.los_observation_discharge_emergency
from
	(
	SELECT
		DISTINCT patient_id,
...
select
	...
	jaccard_similarity,
	round(avg(dur_trace),
	2) as mean_dur_trace_days,
	round(stddev(dur_trace),
	2) as std_dur_trace_days,
	round(median(dur_trace),
	2) as median_dur_trace_days,
	round((QUANTILE_CONT(dur_trace,
	0.75)-QUANTILE_CONT(dur_trace,
	0.25)),
	2) as iqr_dur_trace_days,
	...
	round(avg(los_hospital),
	2) as mean_los_hospital_days,
	round(stddev(los_hospital),
	2) as std_los_hospital_days,
	round(median(los_hospital),
	2) as median_los_hospital_days,
	round((QUANTILE_CONT(los_hospital,
	0.75)-QUANTILE_CONT(los_hospital,
	0.25)),
	2) as iqr_los_hospital_days,
	round(avg(los_observation_discharge_emergency),
	2) as mean_los_observation_discharge_emergency_days,
	round(stddev(los_observation_discharge_emergency),
	2) as std_los_observation_discharge_emergency_days,
	round(median(los_observation_discharge_emergency),
	2) as median_los_observation_discharge_emergency_days,
	round((QUANTILE_CONT(los_observation_discharge_emergency,
	0.75)-QUANTILE_CONT(los_observation_discharge_emergency,
	0.25)),
	2) as iqr_los_observation_discharge_emergency_days,
from
	traces_info
group by
	trace,
...
#  df_model$sex_cd[df_model$sex_cd %in% '1'] <- 'Male'
#  df_model$sex_cd[df_model$sex_cd %in% '2'] <- 'Female'
...
df_model_plot <- df_model %>% dplyr::select(intervention_bl,intervention, exitus_bl, hospital_cd , age_nm , sex_cd , zip_code_cd , hospital_type_discharge_cd , ct_inhospital_bl , mri_inhospital_bl , n_admission_prior_emergency_nm , n_admission_prior_inhospital_nm ,  antiarrhythmics_prescription_bl , antihypertensive_prescription_bl , antiaggregants_prescription_bl , fibrinolitics_prescriptions_bl ,  modified_rankin_scale_cd ,  heart_failure_bl , hypertension_bl , diabetes_bl , atrial_fibrillation_bl , valvular_disease_bl , rank_trace , jaccard_similarity , dur_trace , period , holiday_bl , weekend_bl , weekday,length_of_stay_hospital, exitus_1year_from_start_futimedt_bl)

p_ <- as.data.frame(sum_model_surv$coefficients)
p1_ <- p_ %>% filter(`Pr(>|z|)` <= 0.01)

aux1_ <- data.frame(variables_df = colnames(df_model_plot))
aux2_ <- data.frame(variables_model = rownames(p1_))
aux3_ <- data.frame(var=str_extract(aux2_$variables_model,str_c(aux1_$variables_df,collapse = '|')))
aux3_ <- aux3_ %>% filter(!is.na(var))

aux3_ <- unique(aux3_$var)
...
tryCatch(
  ...
  forest_model(model_surv, covariates = aux3_)
...
rm(aux1_,aux2_,aux3_,p_,p1_,df_model_plot)
#### Propension to intervention model
