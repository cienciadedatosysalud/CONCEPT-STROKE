...
df <- dbGetQuery(conn = con, "SELECT a.*,b.rank_trace,b.jaccard_similarity,b.dur_trace,b.perc FROM main.patient_view a LEFT JOIN main.prediction_outcomes_db b ON a.patient_id=b.patient_id")
...
model_surv <- coxph(surv_obj ~ intervention + hospital_cd + age_nm + sex_cd + zip_code_cd + hospital_type_discharge_cd + ct_inhospital_bl + mri_inhospital_bl + n_admission_prior_emergency_nm + n_admission_prior_inhospital_nm +  antiarrhythmics_prescription_bl + antihypertensive_prescription_bl + antiaggregants_prescription_bl + fibrinolitics_prescriptions_bl +  modified_rankin_scale_cd +  heart_failure_bl + hypertension_bl + diabetes_bl + atrial_fibrillation_bl + valvular_disease_bl + rank_trace + jaccard_similarity + dur_trace + period + holiday_bl + weekend_bl + weekday, data = df_model)
...
df_model_ <- df_model_ %>% rename(`fibrinolysis intervention bl` = intervention_fibrinolysis_bl, `thrombectomy mechanic intervention bl` = intervention_thrombectomy_mec_bl, `combined intervention bl` = intervention_combined_bl, `exitus bl` = exitus_bl)
```
...
tryCatch(
  {
    formula_propensity_model_global <- as.formula(paste0('intervention_bl ~ ',formula_model))
...
tryCatch(
  {
    formula_propensity_model_fibri <- as.formula(paste0('`fibrinolysis intervention bl` ~ ',formula_model))
...
tryCatch(
  {
    formula_propensity_thrombectomy_mec <- as.formula(paste0('`thrombectomy mechanic intervention bl` ~ ',formula_model))
...
tryCatch(
  {
    formula_propensity_combined <- as.formula(paste0('`combined intervention bl` ~ ',formula_model))
...
```{r, echo = FALSE, warning=FALSE}
#| label: prediction outcomes ps_throm model
