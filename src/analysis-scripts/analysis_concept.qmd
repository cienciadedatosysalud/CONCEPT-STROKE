...
```{python, output = FALSE, echo = FALSE}
#| label: python functions
def import_csv_and_convert_to_event_log(data_path):
  con = duckdb.connect(data_path)
  df = con.sql("SELECT * from main.patient").df()
  ...
  df_origin = df
  df.loc[df["inhospital_thrombectomy_bl"] == 1, "inhospital_thrombectomy_bl_date_dt"] = df["hospital_intervention_date_dt"]
  df.loc[df["inhospital_fibrinolysis_bl"] == 1, "inhospital_fibrinolysis_bl_date_dt"] = df["hospital_intervention_date_dt"]
  ...
  plt.close()
  df_out=df.merge(df_origin, left_on='case:concept:name', right_on='patient_id', how='left')
  df_out.to_csv('../../outputs/event_log.csv',sep='|', index=False)
  con.close()
...
# filter by most frequent traces
...
def get_decision_mining(df,event_log):
  df_carac = df[['patient_id','age_nm','sex_cd','hospital_cd','healthcare_area_cd','zip_code_cd', 'municipality_code_cd','type_admission_cd','heart_failure_bl','atc_code_antiaggregants_cd', 'barthel_index_nm']]
  ...
  net, im, fm = pm4py.discover_petri_net_inductive(event_carac, noise_threshold=0.1)
  ...
  return event_carac, point_decision


# Multinomial model to predict complete traces
...
def survival_analysis(df_filtered):
  ...
  df_filtered['survival_in_days'] = (pd.to_datetime(end_study) - df_filtered['admission_emergency_care_dt']) / np.timedelta64(1, 'D')
  df_filtered.loc[df_filtered['exitus_dt'].notna(), 'survival_in_days'] = (df_filtered['exitus_dt'] - df_filtered['admission_emergency_care_dt']) / np.timedelta64(1, 'D')
...
model_gam <- function(file){
df_gam <- read.csv(file = file, sep = '|') %>% dplyr::select(!contains(c('patient_id',"_dt", 'atc')))
...
event_log, traces, freq_traces, df = import_csv_and_convert_to_event_log("../../inputs/data.duckdb")
...
event_carac, point_decision = get_decision_mining(df,event_log)
...
```{r, warning = FALSE, output = FALSE}
  df <- read.csv('../../outputs/event_log.csv', sep = '|') %>% rename(activity = concept.name, timestamp = time.timestamp) %>% 
...
#| label: gamm model call function
model <- model_gam('../../outputs/df_gam.csv')
file.remove('../../outputs/df_gam.csv')
